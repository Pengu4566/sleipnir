def selector_check(df_selector):
    ...
    df_selector_dup['filePath'] = df_selector_dup.apply(lambda x: x['filePath'].replace(x['mainFolder'], ''), axis=1)
    df_selector_dup.selectorStr = df_selector_dup.selectorStr.str.replace("\"", "'")

    if len(df_selector_dup.filePath) > 0:
        return list(df_selector_dup.loc[:, ['selectorStr', 'filePath', 'projectId']]
